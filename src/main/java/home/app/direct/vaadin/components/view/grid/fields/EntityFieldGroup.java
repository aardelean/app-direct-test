...
import home.app.direct.vaadin.components.view.convertors.SimpleStringToIntegerConvertor;
...
public abstract class EntityFieldGroup<T> extends BeanFieldGroup {
	...
	public Field productCodeField(String fieldId, Field field) {
		if(fieldId.equalsIgnoreCase(EntityFieldGroup.PRODUCT_CODE)){
			field = FieldFactory.productCodeField();
		}
		return field;
	}

	public void dateFields(String fieldId, Field field) {
		if(fieldId.equalsIgnoreCase(endValidityDatePropertyName)){
			FieldFactory.validToField((DateField) field);
		}
		if(fieldId.equalsIgnoreCase(startValidityDatePropertyName)){
			FieldFactory.validFromField((DateField) field);
		}
	}
